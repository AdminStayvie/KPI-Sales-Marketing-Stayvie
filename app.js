...
function handleCanvasingForm(e) { e.preventDefault(); const formData = new FormData(e.target); const data = { id: Date.now(), sales: currentUser.username, meetingTitle: formData.get('meetingTitle'), notes: formData.get('notes'), date: getCurrentDateString(), timestamp: getLocalTimestampString() }; sendData('saveData', 'Canvasing', data, e.target.querySelector('input[type="file"]'), e); }
function handlePromosiForm(e) { e.preventDefault(); if (!isWithinCutoffTime()) { showMessage('Batas waktu input harian (16:00) terlewati!', 'error'); return; } const formData = new FormData(e.target); const data = { id: Date.now(), sales: currentUser.username, campaignName: formData.get('campaignName'), platform: formData.get('platform'), date: getCurrentDateString(), timestamp: getLocalTimestampString() }; sendData('saveData', 'Promosi', data, e.target.querySelector('input[type="file"]'), e); }
function handleDoorToDoorForm(e) { e.preventDefault(); const formData = new FormData(e.target); const data = { id: Date.now(), sales: currentUser.username, visitDate: formData.get('visitDate'), institutionName: formData.get('institutionName'), address: formData.get('address'), picName: formData.get('picName'), picPhone: formData.get('picPhone'), response: formData.get('response'), timestamp: getLocalTimestampString() }; sendData('saveData', 'DoorToDoor', data, e.target.querySelector('input[type="file"]'), e); }
function handleQuotationForm(e) { e.preventDefault(); const formData = new FormData(e.target); const data = { id: Date.now(), sales: currentUser.username, customerName: formData.get('customerName'), productType: formData.get('productType'), quotationAmount: formData.get('quotationAmount'), description: formData.get('description'), date: getCurrentDateString(), timestamp: getLocalTimestampString() }; sendData('saveData', 'Quotations', data, e.target.querySelector('input[type="file"]'), e); }
function handleSurveyForm(e) { e.preventDefault(); const formData = new FormData(e.target); const data = { id: Date.now(), sales: currentUser.username, customerName: formData.get('customerName'), gender: formData.get('gender'), phone: formData.get('phone'), surveyDate: formData.get('surveyDate'), origin: formData.get('origin'), feedback: formData.get('feedback'), timestamp: getLocalTimestampString() }; sendData('saveData', 'Surveys', data, e.target.querySelector('input[type="file"]'), e); }
function handleLaporanForm(e) { e.preventDefault(); const formData = new FormData(e.target); const data = { id: Date.now(), sales: currentUser.username, reportPeriod: formData.get('reportPeriod'), managementFeedback: formData.get('managementFeedback'), additionalNotes: formData.get('additionalNotes'), timestamp: getLocalTimestampString() }; sendData('saveData', 'Reports', data, e.target.querySelector('input[type="file"]'), e); }
function handleCrmSurveyForm(e) { e.preventDefault(); const formData = new FormData(e.target); const data = { id: Date.now(), sales: currentUser.username, competitorName: formData.get('competitorName'), website: formData.get('website'), product: formData.get('product'), priceDetails: formData.get('priceDetails'), timestamp: getLocalTimestampString() }; sendData('saveData', 'CRMSurveys', data, null, e); }
function handleKonversiForm(e) { e.preventDefault(); const formData = new FormData(e.target); const data = { id: Date.now(), sales: currentUser.username, eventName: formData.get('eventName'), clientName: formData.get('clientName'), eventDate: formData.get('eventDate'), venueType: formData.get('venueType'), barterValue: formData.get('barterValue'), barterDescription: formData.get('barterDescription'), timestamp: getLocalTimestampString() }; sendData('saveData', 'Conversions', data, null, e); }
function handleEventForm(e) { e.preventDefault(); const formData = new FormData(e.target); const data = { id: Date.now(), sales: currentUser.username, eventName: formData.get('eventName'), eventType: formData.get('eventType'), eventDate: formData.get('eventDate'), eventLocation: formData.get('eventLocation'), organizer: formData.get('organizer'), benefits: formData.get('benefits'), timestamp: getLocalTimestampString() }; sendData('saveData', 'Events', data, e.target.querySelector('input[type="file"]'), e); }
function handleCampaignForm(e) { e.preventDefault(); const formData = new FormData(e.target); const data = { id: Date.now(), sales: currentUser.username, campaignTitle: formData.get('campaignTitle'), targetMarket: formData.get('targetMarket'), campaignDate: formData.get('campaignDate'), conceptDescription: formData.get('conceptDescription'), potentialConversion: formData.get('potentialConversion'), budget: formData.get('budget'), timestamp: getLocalTimestampString() }; sendData('saveData', 'Campaigns', data, e.target.querySelector('input[type="file"]'), e); }
...
function calculateWeeklyAchievements(weekStart) { const weekEnd = new Date(weekStart); weekEnd.setDate(weekEnd.getDate() + 6); const canvasingThisWeek = getFilteredData('canvasing').filter(c => c.date && new Date(c.date) >= weekStart && new Date(c.date) <= weekEnd).length; const doorToDoorThisWeek = getFilteredData('doorToDoor').filter(d => d.visitDate && new Date(d.visitDate) >= weekStart && new Date(d.visitDate) <= weekEnd).length; return canvasingThisWeek + doorToDoorThisWeek; }
function calculateMonthlyAchievements(monthStart) { const campaignsThisMonth = getFilteredData('campaigns').filter(c => c.campaignDate && new Date(c.campaignDate).getMonth() === monthStart.getMonth()).length; const eventsThisMonth = getFilteredData('events').filter(e => e.eventDate && new Date(e.eventDate).getMonth() === monthStart.getMonth()).length; return campaignsThisMonth + eventsThisMonth; }
function updateTargetBreakdown() { const container = document.getElementById('targetBreakdown'); if (!container) return; container.innerHTML = '<div class="target-breakdown"></div>'; const breakdownContainer = container.querySelector('.target-breakdown'); const allTargets = [...appData.daily_targets.map(t => ({ ...t, type: 'Harian' })), ...appData.weekly_targets.map(t => ({ ...t, type: 'Mingguan' })), ...appData.monthly_targets.map(t => ({ ...t, type: 'Bulanan' }))]; let currentType = ''; allTargets.forEach(target => { if (currentData.settings[target.id]) { if (target.type !== currentType) { currentType = target.type; const h4 = document.createElement('h4'); h4.textContent = `Target ${currentType}`; breakdownContainer.appendChild(h4); } const achieved = 0; const status = achieved >= target.target ? 'completed' : 'pending'; breakdownContainer.innerHTML += `<div class="target-item"><div class="target-name">${target.name}</div><div class="target-progress"><span>${achieved}/${target.target}</span><span class="target-status ${status}">${status === 'completed' ? 'Selesai' : 'Pending'}</span></div></div>`; } }); }
function createSummaryTable(containerId, data, headers, rowGenerator) { const container = document.getElementById(containerId); if (!container) return; const userSpecificData = currentUser.role === 'management' ? data : data.filter(d => d.sales === currentUser.username); if (userSpecificData.length === 0) { container.innerHTML = `<div class="empty-state">Belum ada data yang diinput</div>`; return; } let tableHTML = `<table><thead><tr><th>${headers.join('</th><th>')}</th></tr></thead><tbody>`; userSpecificData.slice(-10).reverse().forEach(item => { tableHTML += `<tr>${rowGenerator(item)}</tr>`; }); tableHTML += `</tbody></table>`; container.innerHTML = tableHTML; }
function updateLeadsSummary() { createSummaryTable('leadSummary', currentData.leads, ['Customer', 'Sumber', 'Produk', 'Kontak', 'Waktu'], item => `<td>${item.customerName || ''}</td><td>${item.leadSource || ''}</td><td>${item.product || ''}</td><td>${item.contact || ''}</td><td>${item.timestamp ? new Date(item.timestamp).toLocaleTimeString('id-ID') : ''}</td>`); }
function updateCanvasingSummary() { createSummaryTable('canvasingSummary', currentData.canvasing, ['Judul Meeting', 'Tanggal', 'File', 'Catatan'], item => `<td>${item.meetingTitle || ''}</td><td>${item.date ? formatDate(item.date) : ''}</td><td>${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank">${item.fileName}</a>` : 'N/A'}</td><td>${item.notes || '-'}</td>`); }
function updatePromosiSummary() { createSummaryTable('promosiSummary', currentData.promosi, ['Campaign', 'Platform', 'Tanggal', 'Screenshot'], item => `<td>${item.campaignName || ''}</td><td>${item.platform || ''}</td><td>${item.date ? formatDate(item.date) : ''}</td><td>${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank">${item.fileName}</a>` : 'N/A'}</td>`); }
function updateDoorToDoorSummary() { createSummaryTable('doorToDoorSummary', currentData.doorToDoor, ['Tanggal', 'Instansi', 'PIC', 'Response', 'Bukti'], item => `<td>${item.visitDate ? formatDate(item.visitDate) : ''}</td><td>${item.institutionName || ''}</td><td>${item.picName || ''}</td><td>${(item.response || '').substring(0, 30)}...</td><td>${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank">${item.fileName}</a>` : 'N/A'}</td>`); }
function updateQuotationsSummary() { createSummaryTable('quotationSummary', currentData.quotations, ['Customer', 'Produk', 'Nominal', 'Tanggal', 'File'], item => `<td>${item.customerName || ''}</td><td>${item.productType || ''}</td><td>${item.quotationAmount ? formatCurrency(item.quotationAmount) : ''}</td><td>${item.date ? formatDate(item.date) : ''}</td><td>${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank">${item.fileName}</a>` : 'N/A'}</td>`); }
function updateSurveysSummary() { createSummaryTable('surveySummary', currentData.surveys, ['Customer', 'Tanggal', 'Asal', 'Feedback', 'Dok.'], item => `<td>${item.customerName || ''}</td><td>${item.surveyDate ? formatDate(item.surveyDate) : ''}</td><td>${item.origin || ''}</td><td>${(item.feedback || '').substring(0, 30)}...</td><td>${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank">${item.fileName}</a>` : 'N/A'}</td>`); }
function updateReportsSummary() { createSummaryTable('laporanSummary', currentData.reports, ['Periode', 'File', 'Feedback', 'Upload'], item => `<td>${item.reportPeriod || ''}</td><td>${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank">${item.fileName}</a>` : 'N/A'}</td><td>${((item.managementFeedback || '-') || '').substring(0, 30)}...</td><td>${item.timestamp ? formatDate(item.timestamp) : ''}</td>`); }
function updateCRMSurveysSummary() { createSummaryTable('crmSurveySummary', currentData.crmSurveys, ['Kompetitor', 'Website', 'Produk', 'Harga', 'Tanggal'], item => `<td>${item.competitorName || ''}</td><td>${item.website ? `<a href="${item.website}" target="_blank">Link</a>` : '-'}</td><td>${(item.product || '').substring(0, 20)}...</td><td>${(item.priceDetails || '').substring(0, 20)}...</td><td>${item.timestamp ? formatDate(item.timestamp) : ''}</td>`); }
function updateConversionsSummary() { createSummaryTable('konversiSummary', currentData.conversions, ['Event', 'Client', 'Venue', 'Nilai Barter', 'Tanggal'], item => `<td>${item.eventName || ''}</td><td>${item.clientName || ''}</td><td>${item.venueType || ''}</td><td>${item.barterValue ? formatCurrency(item.barterValue) : ''}</td><td>${item.eventDate ? formatDate(item.eventDate) : ''}</td>`); }
function updateEventsSummary() { createSummaryTable('eventSummary', currentData.events, ['Event', 'Jenis', 'Tanggal', 'Lokasi', 'Dok.'], item => `<td>${item.eventName || ''}</td><td>${item.eventType || ''}</td><td>${item.eventDate ? formatDate(item.eventDate) : ''}</td><td>${item.eventLocation || ''}</td><td>${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank">${item.fileName}</a>` : 'N/A'}</td>`); }
function updateCampaignsSummary() { createSummaryTable('campaignSummary', currentData.campaigns, ['Campaign', 'Target', 'Tanggal', 'Budget', 'Materi'], item => `<td>${item.campaignTitle || ''}</td><td>${item.targetMarket || ''}</td><td>${item.campaignDate ? formatDate(item.campaignDate) : ''}</td><td>${item.budget ? formatCurrency(item.budget) : ''}</td><td>${item.fileUrl ? `<a href="${item.fileUrl}" target="_blank">${item.fileName}</a>` : 'N/A'}</td>`); }
...
document.addEventListener('DOMContentLoaded', function() {
    if(!currentUser) return; // Stop execution if no user
